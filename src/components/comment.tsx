...
function CommentAvatar({ className, ...props }: CommentAvatarProps) {
  return (
    ...
    <img className={cn("size-8 rounded-full", className)} alt="" {...props} />
  )
}
...
function CommentContent({ className, ...props }: CommentContentProps) {
  return (
    <div className={cn("flex-1 px-3 py-2.5 rounded-lg bg-navy-700 border-[0.5px] border-navy-600 flex flex-col gap-1", className)} {...props} />
  )
}
...
function CommentText({ className, ...props }: CommentTextProps) {
  return (
    <p className={cn("text-sm leading-relaxed text-navy-100", className)} {...props} />
  )
}
